...
export default function AdminDashboard() {
  ...
  const [deleteDoor, setDeleteDoor] = useState<{ doorId: string } | null>(null)
  ...
  const handleUpdateDoor = async (doorId: string, newName: string) => {
    try {
      ...
      setDoors(
        doors.map((door) =>
          door._id === doorId ? { ...door, name: newName } : door
        )
      );
  ...
  const handleChangePassword = async (
    ...
    try {
      setIsLoading(true);
      await axios.put(
        `${baseUrl}/devices/changes-password`,
        { roomId: doorId, deviceId, password: newPassword },
        authHeader()
      );
      setDoors(
        doors.map((door) =>
          door._id === doorId
            ? {
                ...
                authData: door.authData.map((auth) =>
                  auth.data === deviceId ? { ...auth, data: newPassword } : auth
                ),
              }
            : door
        )
  ...
  const handleUpdateDevice = async (
    ...
    try {
      ...
      await axios.put(
        ...
        { roomId: doorId, deviceId, nameUser: newName, status: newStatus },
        authHeader()
      );
      setDoors(
        doors.map((door) =>
          door._id === doorId
            ? {
                ...
                authData: door.authData.map((auth) =>
                  auth.data === deviceId
                    ? { ...auth, name: newName, status: newStatus }
                    : auth
                ),
              }
            : door
  ...
  const handleDeleteDevice = async (doorId: string, deviceIndex: number) => {
    try {
      ...
      await axios.delete(
        `${baseUrl}/devices/delete-device`,
        {
          data: { roomId: doorId, data: deviceToDelete.data },
          ...authHeader(),
        }
      );
      ...
      setDoors(
        doors.map((door) =>
          door._id === doorId
            ? {
                ...door,
                authData: door.authData.filter((_, i) => i !== deviceIndex),
              }
            : door
        )
      );
  ...
  const handleDeleteDoor = async (doorId: string) => {
    try {
      ...
      const error = err as AxiosError<{ message?: string }>;
      setError(error.response?.data?.message || "Failed to delete door");
  ...
  return (
    <>
            ...
            <TableBody>
              {doors.flatMap((door) => [
                <TableRow key={`${door._id}-main`}>
                  ...
                  <TableCell>{new Date(door.createdAt).toLocaleDateString()}</TableCell>
                  <TableCell className="text-right">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => setDeleteDoor({ doorId: door._id })}
      ...
      <Dialog
        open={isUpdateDoorOpen}
        onOpenChange={(open) => {
          setIsUpdateDoorOpen(open);
          if (!open) setUpdateDoorId(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
      ...
      <Dialog
        open={isUpdateDeviceOpen}
        onOpenChange={(open) => {
          setIsUpdateDeviceOpen(open);
          if (!open) setUpdateDeviceData(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
      ...
      <Dialog
        open={isUpdatePasswordDeviceOpen}
        onOpenChange={(open) => {
          setIsUpdatePasswordDeviceOpen(open);
          if (!open) setUpdateDeviceData(null);
        }}
      >
        <DialogContent>
      ...
      <AlertDialog open={!!deleteDoor} onOpenChange={() => setDeleteDoor(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete this door. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (deleteDoor) {
                  handleDeleteDoor(deleteDoor.doorId); // Sử dụng deleteDoor.doorId thay vì deleteDevice.doorId
                }
              }}
            >
              Delete
