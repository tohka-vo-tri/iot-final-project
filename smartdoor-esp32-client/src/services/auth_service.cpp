...
String handle_api_request(const String &endpoint, const JsonDocument &payload, const HttpMethod method = HttpMethod::POST)
{
...
void handle_fingerprint_register(const String &payload)
{
    ...
    if (!fingerprint.isEmpty())
    {
        ...
        print_to_lcd(0, "Please Wait...");
        build_auth_payload(jsonDoc, "fingerprint", fingerprint);
        String response = handle_api_request("/devices/add-fingerprint", jsonDoc, HttpMethod::PUT);
        handle_register_response(response, [](const bool &status)
                            {
            if (status) {
...
void handle_rfid_register(const String &payload)
{
    ...
    if (!rfid.isEmpty())
    {
        ...
        print_to_lcd(0, "Please Wait...");
        build_auth_payload(jsonDoc, "rfid", rfid);
        String response = handle_api_request("/devices/add-rfid", jsonDoc, HttpMethod::PUT);
        handle_register_response(response, [](const bool &status)
                            {
            if (status) {
                print_to_lcd(0, "Register Success");
